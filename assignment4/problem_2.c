 ...
 *  
 * Problem 2: Single threaded program that takes a name of a directory as an argument. 
 *  Requirements:
...
int main(int argc, char *argv[]) {
  ...
  numFiles = scandir(argv[1], &fileList, NULL, alphasort);
  
  // Check to see if directory exists and scandir succeeded
  if(numFiles < 0) {
    printf("ERROR: scandir call failed. Check directory name. Exiting...\n");
    return -1;
  }
    ...
    while(n < numFiles) {
      if(fileList[n]->d_type == DT_REG) { // Only process regular files 
  ...
  if(currentFile == NULL) { // Check if file opened correctly
    ...
    // CRC for file size 0 is 0
    if(fileLength == 0) {
      crc = 0;
      printf("%s %08X\n", fileList[n]->d_name, crc);
    }
      ...
      if(fileContents == NULL) {
        printf("ERROR: malloc for file contents buffer failed! Exiting...\n");
      }
      error = fread(fileContents, sizeof(char), fileLength, currentFile);
      if(error == 0) {
        printf("ERROR: fread failed. Exiting...\n");
        return -1;
      }
